...
description: >
  ...
  Łączy skill playwright-cli do browser automation na portalu EKW (ekw.ms.gov.pl, wymaga
  ręcznego rozwiązania CAPTCHA) z REST API ULDK GUGiK do wzbogacenia o geometrię działki
  i współrzędne.
---
...
### Etap 1 — EKW przez Playwright CLI skill

Użyj skilla `/playwright-cli` do automatyzacji przeglądarki:

1. Uruchom skill: `/playwright-cli` z instrukcją nawigacji do
   `https://przegladarka-ekw.ms.gov.pl/eukw_prz/KsiegiWieczyste/wyszukiwanieKW`
2. Poproś skill aby:
   - Wypełnił 3 pola formularza: kod wydziału, numer (8 cyfr), cyfra kontrolna
   - Kliknął "Wyszukaj Księgę"
   - Czekał na wczytanie się wyników (portal może pokazać CAPTCHA — jeśli tak,
     **poproś użytkownika o jej rozwiązanie**, skill zaczeka)
3. Po załadowaniu strony wyników: weź screenshot i parsuj typ KW (grunt / lokal / budynek / SUW) oraz sąd — zapisz do JSON.
4. Poproś skill aby:
   - Kliknął "Przeglądanie aktualnej treści KW" → czekał na załadowanie strony `/pokazWydruk`
   - Widoczne będą zakładki działów (przyciski: `Dział I-O`, `Dział I-Sp`, `Dział II`, `Dział III`, `Dział IV`)
5. Dla każdego działu z listy `--dzialy`:
   - Skill klika odpowiedni przycisk (`Dział I-O`, `Dział I-Sp`, itd.)
   - Bierze screenshot zawartości
   - Prześlij screenshot do siebie (Claude) — ty go parsjesz lokalnie i ekstrakujesz dane
   - Skill przechodzi do następnego działu bez resetowania sesji (ta sama przeglądarka)
   - Między działami nie potrzeba dodatkowego `sleep`, to ta sama sesja
...
Jeśli skill playwright-cli nie jest dostępny lub użytkownik odmówi rozwiązania CAPTCHA:
